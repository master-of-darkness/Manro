...
class PlayerActionMap final : public Manro::IInputActionMap {
public:
    struct Settings {
    ...
    explicit PlayerActionMap(Manro::InputManager &inputMgr)
        : PlayerActionMap(inputMgr, Settings{}) {
    }

    explicit PlayerActionMap(Manro::InputManager &inputMgr, Settings s)
        : m_Input(inputMgr), m_Settings(s) {
    }

    void BuildUserCmd(Manro::UserCmd &cmd) override {
        using K = Manro::Key;
        ...
        Manro::RawMouseDelta delta = m_Input.ConsumeMouseDelta();
        ...
        // cmd.SetButton(Manro::ButtonBit::Jump, m_Input.IsKeyDown(K::Space)); TODO: fix me
        cmd.SetButton(Manro::ButtonBit::Crouch, m_Input.IsKeyDown(K::LeftShift));
    }
